...
-p D:\Projects\Indiana\Dashboards-Plugin-EWS\Database\EdFi.Dashboards.Db.Dashboard.Plugin.EWS\Post-Deployment -s . \
-d IN_EdFi_Dashboard_Daily -u edfiPService -w edfiPService
    """)
...
def get_metric_sql():
    metric_statement = """ 
        select distinct m.MetricId, m.MetricName, pm.MetricName from metric.Metric m
        inner join metric.MetricNode mn on m.MetricId = mn.MetricId
        inner join metric.MetricNode pn on mn.ParentNodeId = pn.MetricNodeId
        inner join metric.Metric pm on pn.MetricId = pm.MetricId
        where m.MetricId in (
        ...
        ) and m.MetricId not in (select distinct MetricVariantId from metric.MetadataListColumn)
         and m.DomainEntityTypeId = 1
    """
...
def get_insert_statement(metric, metadata_list_column_id, group_id, column_order):
    ...
    metric_id, metric_name, metric_prefix = metric
    is_visible_by_default = 1
    is_fixed_column = 0
    metric_cell_type = 3
    insert_stmt = f"""
INSERT [metric].[MetadataListColumn] \
...
VALUES \
({metadata_list_column_id}, {group_id}, '{metric_prefix + ' ' + metric_name}', '{metric_prefix}', \
{is_visible_by_default}, {is_fixed_column}, {metric_cell_type}, {metric_id}, {column_order}, NULL, NULL, '', \
{metric_id})
GO"""
    return insert_stmt
